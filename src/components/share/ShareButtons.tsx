...
interface Props {
  /** Custom share text (e.g. quiz score). Falls back to site description. */
  text?: string;
  /** URL to share. Falls back to current page. */
  url?: string;
...
export default function ShareButtons({ text, url, compact = false }: Props) {
  const shareText = text || `${SITE_NAME}で無料TOEIC対策！`;
  ...
  const buttonBase = compact
    ...
    : "inline-flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-lg transition-colors";
  ...
  return (
    <div className={`flex ${compact ? "gap-2" : "gap-3"} flex-wrap`}>
      <a
        ...
        className={`${buttonBase} bg-[#1DA1F2]/10 text-[#1DA1F2] hover:bg-[#1DA1F2]/20`}
      >
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`${buttonBase} bg-[#06C755]/10 text-[#06C755] hover:bg-[#06C755]/20`}
      >
        <svg className={compact ? "w-3.5 h-3.5" : "w-4 h-4"} viewBox="0 0 24 24" fill="currentColor">
